...
app.get("/f2p-game/products", async (req, res) => {
  ...
  if (req.query.title){
    ...
    data.forEach(game => {
      if(game.title === req.query.title){
        res.send(game)
      }
    });
    }
    else if(req.query.about){
      const response = await fetch(`https://www.freetogame.com/api/games?category=${req.query.about}`);
      const data = await response.json();
      res.send(data)
    }
    else{
