...
const app = express();
const port = 8000;
const sql = postgres({ db: "mydb", user: "user", password: "password",port: 5436 });
...
app.get("/products", async (req, res) => {
  ...
  app.get("/products/:id", async (req, res) => {
    ...
    if (req.query.title) {
        ...
        query += ` AND name ILIKE '%${title}%'`; // ILIKE for case-insensitive search
    }

    // Check if about parameter is provided
    if (req.query.about) {
        const about = req.query.about;
        query += ` AND about ILIKE '%${about}%'`; // ILIKE for case-insensitive search
    }
    const product = await sql`
...
// Mettre à jour partiellement un utilisateur par son ID (PATCH)
...
app.post("/paniers", async (req, res) => {
  ...
  if (result.success) {
    ...
    try {
      // Vous devez récupérer le prix du produit depuis la table des produits
      const product = await sql`
      ...
      const total = price * 1.2; // Total avec TVA
